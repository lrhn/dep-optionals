...
$(T_1$ $a_1, \ldots, T_n$ $a_n, [T_{n+1}$ $x_{n+1} = d_1, \ldots, T_{n+k}$ $x_{n+k} = d_k], \{T_{n+k+1}$ $x_{n+k+1} : d_{k+1}, \ldots,  T_{n+k+m}$ $x_{n+k+m} : d_{k+m}\}) => e$
is 

$(T_1 \ldots, T_n, [T_{n+1}, \ldots, T_{n+k}], \{T_{n+k+1}$ $x_{n+1}, \ldots, T_{n+k+m}$ $x_{n+k+m}\})) \rightarrow T_0$, where $T_0$ is the static type of $e$.

\LMHash{}
The static type of a function literal of the form 

$(T_1$ $a_1, \ldots, T_n$ $a_n, [T_{n+1}$ $x_{n+1} = d_1, \ldots, T_{n+k}$ $x_{n+k} = d_k], \{T_{n+k+1}$ $x_{n+k+1} : d_{k+1}, \ldots,  T_{n+k+m}$ $x_{n+k+m} : d_{k+m}\})$ \ASYNC{} $=> e$
is $(T_1 \ldots, T_n, [T_{n+1}, \ldots, T_{n+k}], \{T_{n+k+1}$ $x_{n+k+1}, \ldots, T_{n+k+m}$ $x_{n+k+m}\}) \rightarrow Future<flatten(T_0)>$, where $T_0$ is the static type of $e$ and  $flatten(T)$ is defined as follows:
...
$(T_1$ $a_1, \ldots, T_n$ $a_n, [T_{n+1}$ $x_{n+1} = d_1, \ldots, T_{n+k}$ $x_{n+k}= d_k], \{T_{n+k+1}$ $x_{n+k+1} : d_{k+1}, \ldots,  T_{n+k+m}$ $x_{n+k+m} : d_{k+m}\})\{s\}$

is $(T_1 \ldots, T_n, [T_{n+1}, \ldots, T_{n+k}], \{T_{n+k+1}$ $x_{n+k+1}, \ldots, T_{n+k+m}$ $x_{n+k+m}\}) \rightarrow  \DYNAMIC{}$.

\LMHash{}
The static type of a function literal of the form  

$(T_1$ $a_1, \ldots, T_n$ $a_n, [T_{n+1}$ $x_{n+1} = d_1, \ldots, T_{n+k}$ $x_{n+k}= d_k], \{T_{n+k+1}$ $x_{n+k+1} : d_{k+1}, \ldots,  T_{n+k+m}$ $x_{n+k+m} : d_{k+m}\})$ $ \ASYNC{}$ $\{s\}$
is $(T_1 \ldots, T_n, [T_{n+1}, \ldots, T_{n+k}], \{T_{n+k+1}$ $x_{n+k+1}, \ldots, T_{n+k+m}$ $x_{n+k+m}\}) \rightarrow Future$.

\LMHash{}
The static type of a function literal of the form  

$(T_1$ $a_1, \ldots, T_n$ $a_n, [T_{n+1}$ $x_{n+1} = d_1, \ldots, T_{n+k}$ $x_{n+k}= d_k], \{T_{n+k+1}$ $x_{n+k+1} : d_{k+1}, \ldots,  T_{n+k+m}$ $x_{n+k+m} : d_{k+m}\})$ $ \ASYNC*{}$ $\{s\}$
is $(T_1 \ldots, T_n, [T_{n+1}, \ldots, T_{n+k}], \{T_{n+k+1}$ $x_{n+k+1}, \ldots, T_{n+k+m}$ $x_{n+k+m}\}) \rightarrow Stream$.

\LMHash{}
The static type of a function literal of the form  

$(T_1$ $a_1, \ldots, T_n$ $a_n, [T_{n+1}$ $x_{n+1} = d_1, \ldots, T_{n+k}$ $x_{n+k}= d_k], \{T_{n+k+1}$ $x_{n+k+1} : d_{k+1}, \ldots,  T_{n+k+m}$ $x_{n+k+m} : d_{k+m}\})$ $ \SYNC*{}$ $\{s\}$
is $(T_1 \ldots, T_n, [T_{n+1}, \ldots, T_{n+k}], \{T_{n+k+1}$ $x_{n+k+1}, \ldots, T_{n+k+m}$ $x_{n+k+m}\}) \rightarrow Iterable$.

\LMHash{}
In all of the above cases, whenever $T_i, 1 \le i \le n+k$, is not specified, it is considered to have been specified as  \DYNAMIC{}. If $d_i, 1 \le i \le k+m$ is not specified, it's considered to have been specified as \NULL{}.
...
\LMHash{}
Let $p_1 \ldots p_h$ be the required parametres of $f$,
let $p_{h+1} \ldots p_{h+l}$ be the optional positional parameters of $f$ and
let $q_1 \ldots q_m$ be the optional named parameters of $f$.

\commentary{
We have an argument list consisting of $n$ positional arguments and $k$ named arguments. We have a function with $h$ required parameters, $l$ optional positional parameters and $m$ optional named parameters. The number of positional arguments must be at least as large as the number of required parameters of $f$ ($h$), and no larger than the number of allowed positional  parameters ($h+l$). All named arguments must have a corresponding named parameter.
}

\LMHash{}
If  $n < h$, or $n > h+l$, the method lookup has failed. Furthermore, each $x_i, n+1 \le i \le n+k$,  must have a corresponding named parameter in the set $\{q_1, \ldots, q_m\}$ or the method lookup also fails.  If $v_o$ is an instance of \code{Type} but $o$ is not a constant type literal, then if $m$ is a method that forwards (\ref{functionDeclarations}) to a static method, method lookup fails. Otherwise method lookup has succeeded.
...
Let $p_1 \ldots p_h$ be the required parameters of $f$,  let $p_{h+1} \ldots p_{h+l}$ be the optional positional parameters of $f$ and let $q_1, \ldots, q_m$ be the optional named parameters declared by $f$.

\LMHash{}
If  $n < h$, or $n > h+l$, the method lookup has failed. Furthermore, each $x_i, n+1 \le i \le n+k$,  must have a corresponding named parameter in the set $\{q1, \ldots, q_m\}$ or the method lookup also fails.  Otherwise method lookup has succeeded.
...
\item  
\begin{dartCode}
$(r_1, \ldots, r_n, \[r_{n+1} = q_1 \ldots r_{n+l} = q_l\], \{p_1 : d_1, \ldots , p_k : d_k\})$ \{
  \RETURN{} $ u.m(r_1, \ldots, r_{n+l}, p_1: p_1, \ldots, p_k: p_k);$
\} 
\end{dartCode}
if $f$ is named $m$ and has required parameters $r_1, \ldots, r_n$, $l$ optional positional parameters, $r_{n+1} \ldots r_{n+l}$ with defaults $q_1 \ldots \q_l$ and named parameters $p_1, \ldots, p_k$ with defaults $d_1, \ldots, d_k$.
...
\begin{dartCode}
$(r_1, \ldots, r_n, \[r_{n+1} = q_1, \ldots , r_{n+l} = q_l\], \{p_1 : d_1, \ldots , p_k : d_k\})$ \{
  \RETURN{} \NEW{} $T.m(r_1, \ldots, r_{n+l}, p_1: p_1, \ldots, p_k: p_k);$
\} 
\end{dartCode}

if $f$ is a named constructor with name $m$ that has required parameters $r_1, \ldots, r_n$, optional positional parameters $r_{n+1}, \ldots, r_{n+l}$ with defaults $q_1, \ldots, q_l$, and named parameters $p_1, \ldots, p_k$ with defaults $d_1, \ldots, d_k$.
...
\begin{dartCode}
$(r_1, \ldots, r_n, \[r_{n+1} = q_1, \ldots , r_{n+l} = q_l\], \{p_1 : d_1, \ldots , p_k : d_k\})$ \{
  \RETURN{} \NEW{} $T(r_1, \ldots, r_{n + l}, p_1: p_1, \ldots, p_k: p_k);$
\} 
\end{dartCode}

if $f$ is an anonymous constructor that has required parameters $r_1, \ldots, r_n$, optional positional parameters $r_{n + 1}, \ldots , r_{n+l}$ with defaults $q_1, \ldots , q_l$, and named parameters $p_1, \ldots, p_k$ with defaults $d_1, \ldots, d_k$.
...
\item  
\begin{dartCode}
$(r_1, \ldots, r_n, \[r_{n+1}, \ldots , r_{n+l}\], \{p_1 : d_1, \ldots , p_k : d_k\})$ \{
  \RETURN{} \SUPER$.m(r_1, \ldots, r_{n + l}, p_1: p_1, \ldots, p_k: p_k);$
\} 
\end{dartCode}
if $f$ is named $m$ and has required parameters $r_1, \ldots, r_n$, optional positional parameters $r_{n + 1}, \ldots , r_{n + l}$ with defaults $q_1, \ldots , q_l$, and named parameters $p_1, \ldots, p_k$ with defaults $d_1, \ldots, d_k$.
\end{itemize}
...
\LMHash{}
 The effect of a type alias of the form  \code{\TYPEDEF{} $T$ $id (T_1$ $p_1, \ldots, T_n$ $p_n, [T_{n+1}$ $p_{n+1}, \ldots, T_{n+k}$ $p_{n+k}], \{T_{n+k+1}$ $p_{n+k+1}, \ldots, T_{n+k+m}$ $p_{n+k+m}\})$} declared in a library $L$ is is to introduce the name $id$ into the scope of $L$, bound to the function type $(T_1, \ldots, T_n, [T_{n+1}, \ldots, T_{n+k}], \{T_{n+k+1}$ $p_{n+k+1}, \ldots, T_{n+k+m}$ $p_{n+k+m}\})  \rightarrow T$. Iff no return type is specified, it is taken to be \DYNAMIC{}. Likewise, if a type annotation is omitted on a formal parameter, it is taken to be \DYNAMIC{}.
...
\LMHash{}
Function types have the general form $(T_1, \ldots, T_n, [T_{n+1} \ldots, T_{n+k}], \{T_{x_1}$ $x_1 \ldots, T_{x_m}$ $x_m\}) \rightarrow T$.
...
A function type $(T_1, \ldots T_{n}, [T_{n+1}  \ldots, T_{n+k}],
\{T_{x_1}$ $x_1 \ldots T_{x_m}$ $x_m\}) \rightarrow T$ is a subtype of the
function type $(S_1, \ldots, S_{n'}, [S_{n'+1} \ldots, S_{n'+k'}], \{S_{y_1}$ $y_1 \ldots S_{y_{m'}}$ $y_{m'}\}) \rightarrow S$, if all of the following conditions are met:
\begin{enumerate}
\item Either
\begin{itemize}
\item $S$ is \VOID{}, Or
...
\item $n \le n'$.
\item $n + k \ge n' + k'$.
\item $\forall i \in 1 .. n'+k', T_i \Longleftrightarrow S_i$.
\item $m \ge m'$ and $y_i \in \{x_1,  \ldots, x_m\}, i \in 1 .. m'$.
%\{x_1,  \ldots, x_m\}$ is a superset of $\{y_1,  \ldots, y_m'\}$.
...
$(T_1, \ldots, T_n, [T_{n+1}, \ldots T_{n+k}], \{\}) \rightarrow T <: (T_1, \ldots, T_n, [T_{n+1}, \ldots T_{n+k}]) \rightarrow T$.

$(T_1, \ldots, T_n, [T_{n+1}, \ldots T_{n+k}]) \rightarrow T <: (T_1, \ldots, T_n, [T_{n+1}, \ldots T_{n+k}], \{\}) \rightarrow T$.

$(T_1, \ldots, T_n, [], \{T_{n+1}$ $x_{n+1}, \ldots, T_{n+m}$ $x_{n+m}\}) \rightarrow T <: (T_1, \ldots, T_n, \{T_{n+1}$ $x_{n+1}, \ldots, T_{n+m}$ $x_{n+m}\}) \rightarrow T$.

$(T_1, \ldots, T_n, \{T_{x_{n+1}}$ $x_{n+1}, \ldots, T_{x_{n+m}}$ $x_{n+m}\}) \rightarrow T <: (T_1, \ldots, T_n, [], \{T_{x_{n+1}}$ $x_{n+1}, \ldots, T_{x_{n+m}}$ $x_{n+m}\}) \rightarrow T$.
...
% This feels like it's confusing function syntax with function types.
% If the type of  int foo(int a, int b) is  (int, int, [], {}) -> int
% and (int, int) -> int is just a shorthand for (int, int, [], {}) -> int,
% then these rules are unnecessary.
}
